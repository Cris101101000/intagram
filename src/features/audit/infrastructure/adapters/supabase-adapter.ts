...
export class SupabaseAdapter implements StoragePort {
  ...
  async getLastAudit(username: string): Promise<PreviousAudit | null> {
    ...
    return {
      ...
      metrics: this.normalizeMetrics(data.metrics) as PreviousAudit['metrics'],
  ...
  async getAuditByToken(accessToken: string): Promise<AuditResult | null> {
    ...
    return {
      ...
      healthSignals: this.normalizeHealthSignals(data.health_signals),
  ...
  // La edge function (ig-audit) guardaba metrics con formato {er:{value}, cr:{value}, rvr:{value}}
  // pero el frontend espera {engagementRate, commentRate, reelsViewRate, hasReels}.
  // deno-lint-ignore no-explicit-any
  private normalizeMetrics(raw: any): AuditResult['metrics'] {
    if (raw && typeof raw === 'object' && 'er' in raw && !('engagementRate' in raw)) {
      return {
        engagementRate: raw.er?.value ?? 0,
        commentRate: raw.cr?.value ?? 0,
        reelsViewRate: raw.rvr?.value ?? 0,
        hasReels: (raw.rvr?.value ?? 0) > 0,
      };
    }
    return raw;
  }

  // ── Normalización de health signals ─────────────────────────────
  // La edge function guarda healthSignals con formato diferente al frontend:
  //   formatMix: {photos, reels, carousels, label} vs {distribution: Record, label}
  //   trend: {erDelta, label} vs {changePercent, label}
  //   consistency: {label} vs {stddev, label}
  // deno-lint-ignore no-explicit-any
  private normalizeHealthSignals(raw: any): AuditResult['healthSignals'] {
    if (!raw || typeof raw !== 'object') return raw;

    // Si ya tiene el formato del frontend, no tocar
    if (raw.formatMix?.distribution) return raw;

    // Convertir formatMix de {photos, reels, carousels} a {distribution: Record}
    const formatMix = raw.formatMix;
    if (formatMix && ('photos' in formatMix || 'reels' in formatMix)) {
      raw.formatMix = {
        distribution: {
          Image: formatMix.photos ?? 0,
          Clips: formatMix.reels ?? 0,
          Sidecar: formatMix.carousels ?? 0,
        },
        label: formatMix.label === 'Dependiente' ? 'Dependiente de un formato' : formatMix.label,
      };
    }

    // Convertir trend de {erDelta} a {changePercent}
    const trend = raw.trend;
    if (trend && 'erDelta' in trend && !('changePercent' in trend)) {
      raw.trend = {
        changePercent: trend.erDelta ?? 0,
        label: trend.label,
      };
    }

    // Asegurar que consistency tenga stddev
    const consistency = raw.consistency;
    if (consistency && !('stddev' in consistency)) {
      raw.consistency = {
        stddev: 0,
